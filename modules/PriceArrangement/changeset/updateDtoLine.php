	...
	public function applyChange() {
		...
		if ($this->isApplied()) {
			...
			if ($this->isModuleInstalled($modname)) {
				...
				$cnmsg = $adb->getColumnNames('vtiger_PriceArrangement');
				if (in_array('accountid', $cnmsg)) {
					$query_result = $adb->pquery('SELECT PriceArrangementid, accountid FROM vtiger_PriceArrangement', array());
					while ($_rows = $adb->fetch_array($query_result)) {
						$crmid_PriceArrangement = $_rows['PriceArrangementid'];
						$crmid_accounts = $_rows['accountid'];
						$this->ExecuteQuery(
							'INSERT INTO `vtiger_crmentityrel` (`crmid`, `module`, `relcrmid`, `relmodule`) VALUES (?,?,?,?)',
							array($crmid_PriceArrangement, 'PriceArrangement', $crmid_accounts, 'Accounts')
						);
					}
				}
				$fieldLayout=array(
					'PriceArrangement' => array(
						'LBL_PriceArrangement_INFORMATION'=> array(
							'activestatus' => array(
				...
				$this->ExecuteQuery('update vtiger_PriceArrangement set activestatus=?, returnvalue=?', array('1', 'Unit+Discount'));
